...
本 skill 內容**改自** [`akseolabs-seo/AK-Threads-Booster`](https://github.com/akseolabs-seo/AK-Threads-booster) 的 `knowledge/algorithm.md`(2026-03-31 更新),原作以 MIT 授權釋出。我們把原文幾乎逐字保留,僅加上下面這層 preamble 做術語對照,因為 AK 把規則綁到他自家的 M6.3 / M8 / M5 模組,我們不跑那套。
...
| `[Tracker 欄位] ...` | 我們**沒有 tracker**,當作觀察維度即可 |
...
1. **第一輪先掃紅線(R1–R12)** — 命中任何一條,**直接警告使用者**,不要委婉。紅線是硬規則,踩到會被降權。
2. **第二輪看信號(S1–S14)** — 不是必殺,是「這篇潛力如何」的觀察。語氣是顧問式:「你這篇在 S1 私訊分享潛力上看起來偏弱」,不是命令。
3. **發後追蹤(P1–P4)用發文後** — 文字本體寫完後不是結束,前 3 小時的留言區互動策略也很重要,要主動提醒使用者。
4. **跨平台 / 帳號層級(A1–A2)** — 整體節奏建議,不要爆量、保持人設一致。
...
1. **紅線區**:踩到就直接警告「這個會被降權,你確定要這樣寫嗎」。發前自查時逐條掃描,命中任一項即觸發警告。
2. **信號區**:不是硬規則,是動態參考。發前自查用來評估貼文的演算法對齊程度,發後追蹤用來追蹤各信號在用戶帳號上的實際表現。
