...
## 3. 中文 TTS 發音地雷詞（ElevenLabs V3）

ElevenLabs V3 對以下詞彙有發音問題，寫 voiceover 字串時必須替換：

| 原文 | 替換成 | 原因 |
|------|--------|------|
| `SQL` | `SEQUEL` | 念成字母拼讀 |
| `PostgreSQL` | `Postgre SEQUEL` | 同上 |
| `GB`（容量） | `G B`（字母間加空格） | 念成 "G bye" |
| `Zeabur` | `Zee-bur` | zea = sea 音改 z；bur = number 的 ber |
| `Linode` | `Lin-node` | 確保念成 Lin + node 兩音節 |
| `.dev`（域名） | `點 D E V` | 避免念成英文 "dot dev" |
| `MIT 授權` | `M I T license` | 「授權」被念成「俱全」類似音 |

**規則**：只改 voiceover 字串，**不改** paragraph / code / callout 顯示文字。
